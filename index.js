...
if (AllConfig.Twitch.evalGlobal)
	for (var i = 0; i < 5; i++)
		console.log(
			'[!!!] Having evalGlobal enabled is a terrible idea... For token safety reasons or something idk...'
		);
...
const CG = {
	...
	Clash: class Clash {
		...
		static modeMapping = {f: 'FASTEST', fast: 'FASTEST', fastest: 'FASTEST', s: 'SHORTEST', short: 'SHORTEST', shortest: 'SHORTEST', r: 'REVERSE', rev: 'REVERSE', ver: 'REVERSE', reverse: 'REVERSE', reversimus: 'REVERSE',};
		...
		constructor(modes, langs) {
			this.modes = new Set(
				modes
					.map((mode) => CG.Clash.modeMapping[mode] || '')
					.filter((mode) => mode)
			);
			this.languages = new Set(
				langs
					.map((lang) =>
						CG.Clash.languagesLower.includes(lang.toLowerCase())
							? CG.Clash.languages[
									CG.Clash.languagesLower.indexOf(lang.toLowerCase())
								]
							: ''
					)
					.filter((lang) => lang)
		...
		async create() {
			const req = await CG.Request.POST(
				'https://www.codingame.com/services/ClashOfCode/createPrivateClash',
				[CG.Config.userId, [...this.languages], [...this.modes]]
			);
			const json = await req.json();

			if (req.status != 200) {
				console.error(
					`* [CG] Creating clash failed with code ${req.status}, server returned the following data:`
				);
				console.error(json);
				return false;
			}
		...
		async submit() {
			...
			await CG.Request.POST(
				...
				[
					this.ideHandle,
					{
						code: `Stream: https://twitch.tv/${Twitch.Config.channel}\nBot: https://github.com/rozbrajaczpoziomow/coc-bot-electric-boogaloo`,
						programmingLanguageId:
							CG.Clash.languages[
								Math.floor(Math.random() * CG.Clash.languages.length)
							],
					},
					null,
				]
			...
			await CG.Request.POST(
				'https://www.codingame.com/services/ClashOfCode/shareCodinGamerSolutionByHandle',
				[CG.Config.userId, this.publicHandle]
			);
		}
		...
		static async init() {
			this.languages = await (
				await CG.Request.POST(
					'https://www.codingame.com/services/ProgrammingLanguage/findAllIds',
					[]
				)
			).json();
...
Twitch.Client = new tmi.Client({
	...
	channels: [Twitch.Config.channel],
});

const isAdmin = (username) => {
	return Twitch.Config.admins.includes(username.toLowerCase());
};
...
Twitch.EventListeners = {
	message: async function onMessage(channel, tags, message, self) {
		const send = (msg) =>
			Twitch.Client.say(channel, `@${tags.username} ` + msg);
		// if(self) return;
		...
		switch (cmd.toLowerCase()) {
			case 'help':
				let viewerCommands = ['!help', '!link', '!uptime', '!playlist', '!followage',];
		...
		today is your lucky day: ${viewerCommands.join(', ')}`);
			...
			case 'new':
				if (!isAdmin(tags.username)) return;

				Twitch.CurrentClash = new CG.Clash(args, args);
				if (!(await Twitch.CurrentClash.create())) return send('Creating clash failed (see console)...');
				return send(Twitch.CurrentClash.url);

			case 'start':
				if (!isAdmin(tags.username)) return;
				if (!Twitch.CurrentClash.start) return send(`There hasn't been a clash created.`);
			...
			case 'override':
				...
				if (args.length == 0) return send('No link provided, usage: !override <link>');
				if (args.length > 1) return send('Too many arguments, usage: !override <link>');
			...
			case 'eval':
				...
				if (!Twitch.Config.evalGlobal && !isAdmin(tags.username)) return;
